...
/** A WebGL fibre-optic tunnel that recedes behind the page -- the interactive
 *  background for the loading screen and the login/signup card, replacing
 *  the flat dot grid of CursorWave with something that reads as "connecting"
 *  rather than just decorative motion.
 *
 *  Ported from React Bits' LightTunnel (JS + CSS variant) and trimmed to
 *  this app's needs: colors track the CSS accent tokens and follow the
 *  light/dark toggle the same way CursorWave does, and it honors the same
 *  "no motion for anyone who asked for less" gate -- when reduced motion is
 *  requested the canvas never mounts, only the div's fallback color shows.
 *
 *  No `avoidRef` here, unlike CursorWave: the tunnel fades out well before
 *  the screen edges (see `fadeFar`) and every caller already places an
 *  opaque card on top, so nothing needs to dodge it.
 */
...
export function LightTunnel({
  ...
  useEffect(() => {
    ...
    // The container is pointer-events-none (clicks must reach the card on
    // top of it), so the canvas itself never receives pointer events --
    // track the cursor on window instead, same as CursorWave does.
    ...
    // Mount once; live prop changes flow through the uniform-sync effect below.
  ...
  return (
    <div
      ref={containerRef}
      aria-hidden="true"
      className={`pointer-events-none fixed inset-0 z-0 h-full w-full overflow-hidden ${className}`.trim()}
    />
  );
}
